...
# segons d'espera després d'una descàrrega XTRA, abans d'activar el GNSS
MARGE_POST_XTRA = 10
...
def obtenir_psm_negociat():
    ...
    try:
        ...
        debug("Error parsejant PSM negociat:", error)
        return None, None
...
def publicar_mqtt(posicio, status):
    ...
    # marge perque escoltar_mqtt rebi l'ordre retained abans de desconnectar
    time.sleep(3)
    client.disconnect()
...
def escoltar_mqtt(client):
    try:
        ...
        debug("Error escoltant MQTT:", error)
...
def processar_ordre(topic, missatge):
    ...
    try:
        ...
        debug("Error processant ordre:", error)
...
def executar_ota(fitxers, client):
    ...
    for nom in fitxers:
        ...
        if resultat != 0:
            debug("Error descarregant OTA:", nom, resultat)

            for f in fitxers_baixats:
                try:
                    uos.remove("/usr/" + f)
                except Exception as error:
                    debug("Error netejant fitxer parcial:", f, error)

            client.disconnect()
            return

        fitxers_baixats.append(nom)
...
def main():
    ...
    if stage == 3 and state == 1:
        try:
            ...
            debug("Estat XTRA:", estat_xtra)

            if estat_xtra == "descarregat":
                # Dona marge perque la radio cel·lular quedi lliure
                # abans d'activar el GNSS, evitant interferencies
                time.sleep(MARGE_POST_XTRA)
        except Exception as error:
            debug("Error actualitzant XTRA:", error)
            estat_xtra = "error"
        ...
        if quecgnss.init() != 0:
            ...
            try:
                ...
                debug("Error obtenint posicio:", error)
                posicio = None
        ...
        try:
            ...
            debug("Error publicant MQTT:", error)
