...
def obtenir_posicio():
    _debug("TEMPS_MAXIM_FIX:", config.TEMPS_MAXIM_FIX)
...
def obtenir_fix(wdt=None):
    ...
    temps_gnss = utime.ticks_diff(utime.ticks_ms(), inici) // 1000

    if wdt:
        wdt.feed()

    _debug("Posició obtinguda en", temps_gnss, "segons:", posicio)
